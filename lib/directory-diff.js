...
listingDiff = function(basePath, updatedPath, baseListing, updatedListing){
	return Promise.all(_.compact(_.map(updatedListing, function(updatedFileObj){
		//logger.debug('treating: '+updatedFileObj.name)
		var baseFileObj
		if(baseFileObj = _.find(baseListing, {name:updatedFileObj.name})){
			//logger.debug('Found in both: '+updatedFileObj.name+','+baseFileObj.name)
			...
			return Promise.all([fs.statAsync(baseFilePath),fs.statAsync(updatedFilePath)]).then(function(stats){
				//console.log('comparing '+baseFileObj.name+' and '+updatedFileObj.name+' by size')
				if(stats[0].size != stats[1].size){
					//file has been changed
					return updatedFileObj
				}else{
					//console.log('comparing '+baseFileObj.name+' and '+updatedFileObj.name+' by content')
					//check by contents
					return Promise.all([fs.readFileAsync(baseFilePath),fs.readFileAsync(updatedFilePath)]).then(function(contents){
						//console.log('contents of '+baseFileObj.name +' and '+updatedFileObj.name+": "+contents)
						if(contents[0].toString() != contents[1].toString())
							return updatedFileObj
						//console.log('contents of '+baseFileObj.name +' and '+updatedFileObj.name+" were the same")
