...
module.exports = {
	...
	archive: function(options){
		...
		return (options.directory?
			fs.readdirAsync(options.directory).then(function(files){
				return _.map(files, function(fileName){
					return path.join(options.directory, fileName)
				})
			}):
			Promise.resolve(options.files)).then(function(files){
				logger.info('Archiveing ' + files.length + ' files into '+options.output)
				var command = executable7z+' a -tzip -y "'+options.output+'" "'+files.join('" "')+'"'
				return run(command)
			})
	},
