...
module.exports = function(options){
	...
	tmp.dir({
		...
		Promise.all([
			...
			.then(_.partial(createFolderStructure,diffPath))
			.then(function(listing){
				return Promise.all(_.map(listingToPaths(listing),function(path){
					return run('cp '+path.join(updatedPath,path)+' '+path.join(diffPath,path)).then(function(){
						return path
					})
				})).then(function(){
					return listing
