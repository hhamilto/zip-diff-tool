...
module.exports = function(options){
	...
	//	unsafeCleanup: true
		...
		Promise.all([
			...
			logger.debug("comparing paths:",basePath,',',updatedPath)
			return directoryDiff(basePath, updatedPath)
			.then(function(listing){
				logger.debug(require('util').inspect(listing))
				return listing
			})
			.then(function(listing){
			...
			.then(function(listing){
				console.log(require('util').inspect(listing))
				return Promise.all(_.map(listingToPaths(listing),function(pathString){
					return run('cp '+path.join(updatedPath,pathString)+' '+path.join(diffPath,pathString)).then(function(){
						return pathString
					})
				})).then(function(){
					return listing
